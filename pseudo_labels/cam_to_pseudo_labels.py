...
def _work(process_id, infer_dataset, args):
    ...
    for iter, pack in enumerate(infer_data_loader):
        ...
        try:
            cam_dict = np.load(os.path.join(args.cam_out_dir, img_name.replace('/', '_') + '.npy'), 
            ...
            imageio.imwrite(os.path.join(args.pseudo_labels_out_dir, 
                img_name.replace('/', '_') + '.png'), conf.astype(np.uint8))

            if process_id == args.num_workers - 1 and iter % (len(databin) // 20) == 0:
                print("%d " % ((5 * iter + 1) // (len(databin) // 20)), end='')
        except:
            print("didn't find a npy for {}".format(os.path.join(args.cam_out_dir, 
                img_name.replace('/', '_') + '.npy')))

def run(args):
    dataset = voc12.dataloader.VOC12ImageDataset(args.train_list, voc12_root=args.voc12_root, 
